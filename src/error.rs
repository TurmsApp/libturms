...
impl Error {
    /// Throw an [`Error`].
    pub fn new(etype: ErrorType, cause: Option<BError>, context: Option<String>) -> Self {
        Error {
            etype,
            cause,
            context,
...
impl fmt::Display for IoError {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            IoError::ReadingError => write!(f, "Cannot read file with provided path."),
        }
    }
}
...
impl fmt::Display for TokenError {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            TokenError::Fail => write!(f, "Token cannot be encoded or decoded."),
